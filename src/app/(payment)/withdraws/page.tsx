...
import { ExtenedWithdraw, WithdrawStatus } from "@/type/payment";
...
const Withdraws: React.FC = () => {
  ...
  const [approvalApi, { isLoading: approvalActionLoading }] =
    useApprovalWithdrawMutation();
  ...
  const handleApproval = (
    ...
    const asyncAction = async () => {
      const response = await approvalApi({
        actionType,
        withdrawId,
        message: message,
      }).unwrap();
  ...
  const renderStatusBadge = (status: WithdrawStatus) => {
    switch (status) {
      case "PENDING":
        return (
  ...
  useEffect(() => {
    ...
    } else if (activeTab == "approved" && filter.status !== "APPROVED") {
      ...
      <>
        <div className="overflow-x-auto border rounded-md">
          <Table className="  ">
            ...
            <TableBody>
              {withdraws?.map((withdraw: ExtenedWithdraw) => (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                      ...
                      >
                        ...
                        <SelectContent>
                          {paymentWallets?.map((pw: any, i: number) => (
                            <SelectItem
                              value={pw.walletName}
                              key={i}
                    ...
                    {selectedWithdraw.status === "PENDING" && (
                      <div>
                        ...
                        <Textarea
                          ...
                          placeholder="Enter a message to the user about this withdrawal..."
                          value={message}
                ...
                {selectedWithdraw?.status === "PENDING" ? (
                  <>
                    <Button
                      ...
                      variant="destructive"
                      onClick={() =>
                        handleApproval(selectedWithdraw.id, "reject")
                      }
                      className="!rounded-button whitespace-nowrap cursor-pointer"
                    >
                    ...
                    <Button
                      disabled={approvalActionLoading}
                      onClick={() =>
                        handleApproval(selectedWithdraw.id, "approve")
                      }
